...
const HistoryChart = () => {
    ...
    const historyDataQuery = useQuery<[]>({
        ...
        queryFn: async() => await axios_instance_token.get(`/stats/user-history?timeframe=${timeframe}&month=${period.month}&year=${period.year}`).then(res => {
...
function CustomTooltip({active, payload}: any){
    ...
    const {tickets, resolved, unresolved} = data

    return (
        <div className='min-w-[200px] rounded border bg-background p-4'>
            <TooltipRow label="Tickets" value={tickets} bgColor="bg-[#106981]" textColor="text-[#106981]" />
            <TooltipRow label="Resolved" value={resolved} bgColor="bg-[#7d3c9b]" textColor="text-[#7d3c9b]" />
            <TooltipRow label="Unresolved" value={unresolved} bgColor="bg-[#ff5733]" textColor="text-[#ff5733]" />
        </div>
    )
}
