...
const ClientPicker = ({onChange, defaultValue}:Props) => {
    ...
    const clientsQuery =  useQuery<User[]>({
        ...
        queryFn: async() => await axios_instance_token.get("/users/clients").then(res => res.data)
    })

    const selecteduser = clientsQuery.data?.find((user:User)=> user.id === value)
            ...
            <PopoverContent className='w-[300px] p-0'>
                <Command onSubmit={e=> e.preventDefault()}>
                    ...
                    <CommandGroup>
                        <CommandList>
                            {clientsQuery?.data && 
                                clientsQuery.data.map((user:User) => {
                                    return (
